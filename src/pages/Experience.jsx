...
const Experience = () => {
  ...
  useEffect(() => { fetchExperience().then(setExperiences).catch(console.error); }, []);
  return (<section id="experience" className="section-padding bg-[#0a0a0a]">
        ...
        {experiences?.map((experience) => <article key={experience?.company} className="relative glass-card p-6 sm:p-8">
          ...
          <div className="flex flex-wrap items-start justify-between gap-4">
            <div className="flex flex-row items-center gap-4">
              <Img src={urlFor(experience?.companyLogo)} className="w-18 h-18" />
            ...
            <div className="rounded-full border border-white/10 bg-white/[.04] px-3 py-1.5 text-xs font-semibold text-[#a0a0a0]">{getStartEndDate(experience?.startDate, experience?.endDate)} · {experience.location}</div>
          </div>
          <ul className="mt-5 space-y-2 text-sm leading-6 text-[#a0a0a0] sm:text-base">{experience?.summary?.split('.').map((highlight) => <li key={highlight} className="flex gap-3"><span className="mt-2 h-1.5 w-1.5 shrink-0 rounded-full bg-[#1fbabf]" />{highlight}.</li>)}</ul>
          <div className="mt-6 flex flex-wrap gap-2">{experience?.technologies?.map((skill) => <span className="tech-pill" key={skill}>{skill}</span>)}</div>
        </article>)}
      </div>
    </div>
  </section>)
